...
module "eks-cluster" {
 ...
 public_subnets             = "${element(data.terraform_remote_state.vpc.public_subnets, 0),element(data.terraform_remote_state.vpc.public_subnets, 1)}"
 private_subnets_cidr        ="${element(data.terraform_remote_state.vpc.private_subnets_cidr,0),element(data.terraform_remote_state.vpc.private_subnets_cidr,1) }"
